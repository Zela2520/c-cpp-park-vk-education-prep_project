...
//    reloadTime /= 500;
    ...
    for (int i = 0; i < mobs.size(); ++i) {
        mobs[i].moveMob(mobs[i].setTaregt(players), map->getWalls(), moveTime);
    }
    std::cout << "MOBBBBBBBBB" << "\n";
    for (int i = 0; i < mobs.size(); ++i) {
        int amountOfDeletedMobs = 0;
        for (auto& bullet : bullets) {
            if (mobs[i - amountOfDeletedMobs].getSprite().getGlobalBounds().intersects(bullet.getSprite().getGlobalBounds())) {    //// Если случилось пересечение со стеной
